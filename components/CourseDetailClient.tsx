...
const EnrollmentButton: React.FC<{ price: number; courseTitle: string }> = ({ price, courseTitle }) => {
  return (
    <a
      ...
      className="w-full py-3 bg-blue-900 text-white text-xs font-bold uppercase tracking-widest hover:bg-blue-800 transition-colors text-center block"
    >
      Secure Your Seat
    </a>
  );
    ...
    <div className="pt-20 pb-16 bg-white">
            ...
            <div className="lg:col-span-7 space-y-6">
              <div className="flex flex-wrap gap-2">
                {course.categories.map((cat, i) => (
                  <span key={i} className="px-2 py-0.5 border border-blue-900/20 bg-white text-blue-900 text-[10px] font-bold uppercase tracking-wide">
                    {cat}
                  </span>
                ))}
                <span className="px-2 py-0.5 bg-blue-900 text-white text-[10px] font-bold uppercase tracking-wide">
                  {course.level}
                </span>
              </div>

              <h1 className="text-3xl md:text-5xl font-extrabold text-slate-900 leading-tight uppercase tracking-tight font-sans">
                {course.title}
              </h1>

              <p className="text-slate-600 text-sm md:text-base leading-relaxed font-mono">
                {course.description}
              </p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 py-4 border-y border-slate-200">
                <div>
                  ...
                  <div className="text-sm font-bold text-slate-900 font-mono mt-1">{course.durationInMonths} Months ({course.durationInDays} Days)</div>
                </div>
                <div>
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Rating</div>
                  <div className="text-sm font-bold text-slate-900 font-mono mt-1">★ {course.rating} / 5.0</div>
                </div>
                <div>
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Language</div>
                  <div className="text-sm font-bold text-slate-900 font-mono mt-1 uppercase">{course.language}</div>
                </div>
                <div>
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Tuition</div>
                  <div className="text-sm font-bold text-slate-900 font-mono mt-1">₹{course.price.toLocaleString('en-IN')}</div>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4">
                <a
                  ...
                  className="w-full sm:w-auto h-12 px-8 bg-blue-900 text-white text-xs font-bold tracking-widest uppercase flex items-center justify-center hover:bg-blue-800 transition-colors whitespace-nowrap"
                >
                  Enroll in Program
                </a>
                {course.isaEnabled && (
              ...
              <div className="border border-slate-200 bg-white p-3 relative group">
                <div className="relative aspect-video overflow-hidden bg-slate-900 flex items-center justify-center">
                  ...
                  {course.videoUrl && (
                    ...
                    >
                      <PlayCircle className="w-16 h-16 text-white drop-shadow-md hover:scale-110 transition-transform duration-200" />
                    </a>
                  )}
                </div>
                <div className="p-3 text-center">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Watch Program Introduction</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="px-6 max-w-7xl mx-auto grid lg:grid-cols-12 gap-8 mb-16">
        ...
        <div className="lg:col-span-8 space-y-12">
          ...
          <div className="border border-slate-200 p-6 md:p-8 bg-slate-50/30">
            <h2 className="text-xl font-bold text-slate-900 mb-6 uppercase tracking-tight font-sans">What You Will Learn</h2>
            <div className="grid md:grid-cols-2 gap-4">
              {course.highlights.map((highlight, idx) => (
                <div key={idx} className="flex gap-3 items-start">
                  <CheckCircle className="w-5 h-5 text-green-600 flex-shrink-0 mt-0.5" />
                  <span className="text-slate-700 text-sm font-mono leading-relaxed">{highlight}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-slate-900 uppercase tracking-tight font-sans">Curriculum Syllabus</h2>
              <span className="text-xs font-mono text-slate-500 uppercase">{course.modules.length} Modules</span>
            </div>
                  ...
                  <div key={mod.id} className="border border-slate-200 bg-white overflow-hidden">
                    ...
                    >
                      <div className="flex gap-4 items-start">
                        <div className="text-2xl font-extrabold text-blue-900/30 font-mono leading-none">
                          {String(modIdx + 1).padStart(2, '0')}
                        </div>
                        <div>
                          <h3 className="text-sm md:text-base font-bold text-slate-900 uppercase tracking-tight font-sans">
                            {mod.title}
                          </h3>
                      ...
                      <div className="border-t border-slate-100 p-5 bg-slate-50/50 space-y-6">
                        ...
                        {mod.videoUrl && mod.videoUrl.length > 0 && (
                          <div className="bg-white border border-slate-200 p-4">
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-2">Module Video Classes</span>
                        ...
                        <div className="space-y-3">
                          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">Tasks &amp; Assignments</span>
                              ...
                              <div key={task.id} className="border border-slate-200 bg-white overflow-hidden">
                                ...
                                >
                                  <div className="flex items-center gap-3">
                                    <span className={`px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider font-mono ${task.type === 'MCQ' ? 'bg-purple-50 text-purple-700 border border-purple-200' : 'bg-blue-50 text-blue-900 border border-blue-900/30'
                                      }`}>
                                      {task.type}
                                    </span>
                                  ...
                                  <div className="border-t border-slate-100 p-4 bg-slate-50/30 space-y-4">
                                    ...
                                    {task.videoUrl && (
                                      <a
                                        ...
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="inline-flex items-center gap-1.5 text-[10px] font-bold text-slate-700 hover:text-slate-900 bg-white border border-slate-200 px-3 py-1.5 uppercase tracking-wide transition-colors"
                                      >
                                        <PlayCircle size={12} />
                                        Task Video Tutorial
                                      </a>
                                    ...
                                    {task.type === 'MCQ' && (
                                      <div className="bg-white border border-slate-200 p-4 space-y-3">
                                        <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block">Select Correct Option</span>
                                        <div className="space-y-2">
        ...
        <div className="lg:col-span-4">
          <div className="sticky top-20 border border-slate-200 p-6 bg-slate-50 space-y-6 shadow-sm" id="pricing-section">
            <h3 className="text-base font-bold text-slate-900 uppercase tracking-tight border-b border-slate-200 pb-3 font-sans">
              Enrollment Details
            </h3>

            {/* Price section */}
            <div className="space-y-1">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">Program Fee</span>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-extrabold text-slate-900 font-mono">₹{course.price.toLocaleString('en-IN')}</span>
            ...
            {course.isaEnabled ? (
              <div className="border border-green-200 bg-green-50 p-4 space-y-2">
                <div className="flex items-center gap-2 text-xs font-bold text-green-800 uppercase tracking-wider font-sans">
                  <ShieldCheck className="w-4 h-4 text-green-700" />
                  <span>ISA Pay-After-Placement</span>
                </div>
                <p className="text-[10px] text-green-700 font-mono leading-relaxed">
                  Pay ₹0 upfront fee. Pay only once you secure a job paying at least ₹5 LPA.
                </p>
                <div className="text-[9px] font-bold text-green-800 uppercase font-mono mt-1">
            ...
            {/* Program stats list */}
            <ul className="space-y-3 py-3 text-slate-600 text-xs font-mono border-t border-slate-200">
              ...
              <li className="flex items-center justify-between">
                <span>Language</span>
                <span className="font-bold text-slate-900 uppercase">{course.language}</span>
              </li>
              <li className="flex items-center justify-between">
      ...
      <section className="px-6 max-w-7xl mx-auto mb-16 border-t border-slate-200 pt-16">
        <div className="grid md:grid-cols-2 gap-12 items-center font-mono">
          <div>
            <h2 className="text-xl font-bold text-slate-900 mb-6 uppercase tracking-tight font-sans">Professional Certification</h2>
            ...
            <ul className="space-y-3">
              {[
                ...
                <li key={idx} className="flex items-start gap-3 p-3 border border-slate-200 bg-slate-50">
                  ...
                  <div>
                    <h3 className="text-xs font-bold text-slate-900 uppercase tracking-tight font-sans">{item.title}</h3>
          ...
          <div className="relative">
            <div className="bg-white border border-slate-200 p-6 shadow-sm">
              <div className="border-2 border-double border-slate-200 p-6 text-center">
                ...
                <h3 className="text-base font-bold text-slate-900 mb-2 uppercase tracking-wide font-sans">Certificate of Completion</h3>
                <p className="text-xs text-slate-400 mb-4 font-sans">This is to certify that</p>
                <div className="border-b border-slate-300 mb-3 pb-2">
                  <p className="text-lg font-bold text-slate-900 font-sans">Student Name</p>
                </div>
                ...
                <div className="flex justify-between items-end text-left font-sans">
                  <div>
                    <div className="border-b border-slate-300 w-20 mb-1"></div>
                    <p className="text-[9px] text-slate-400">Date</p>
                  </div>
                  <div className="w-12 h-12 bg-blue-900 flex items-center justify-center text-white text-[7px] font-bold tracking-widest">
                    VERIFIED
                  </div>
